...
if (command === 'mcp') {
  ...
  const t0 = Date.now();
  const pipe = await pipeline('feature-extraction', model, isGemma ? { dtype: 'q8' as any } : {});
...
async function handleTemplatesCommand(): Promise<void> {
  ...
  if (subcommand === 'list') {
    ...
    const manifest = ensureManifest(config.crmRoot);
    let remote: Awaited<ReturnType<typeof listRemoteTemplates>>;
    try {
