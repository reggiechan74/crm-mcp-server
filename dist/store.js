...
export function createStore(dbPath, crmRoot) {
    ...
    `),
        ...
        getContentCache: db.prepare('SELECT * FROM content_cache WHERE contact_id = ? AND section = ?'),
        getAllContentCache: db.prepare('SELECT * FROM content_cache WHERE contact_id = ?'),
        getRelationships: db.prepare('SELECT * FROM relationships WHERE source_id = ? OR target_id = ?'),
    };
    /**
    ...
    function indexDossier(dossierRelPath) {
        ...
        for (const rel of rels) {
            stmts.insertRelationship.run(rel.sourceId, rel.targetId, rel.targetName, rel.type, rel.context, rel.bidirectional ? 1 : 0);
        }
    ...
    const store = {
        ...
        searchContacts(filters) {
            const { query, category, status, profession, limit = 20, } = filters;
            const conditions = [];
            const params = [];
            if (query) {
      ...
      `;
        ...
        getSection(contactId, section) {
            ...
            const cleaned = stripBoilerplate(raw);
            const now = new Date().toISOString();
            stmts.insertContentCache.run(contactId, section, currentHash, cleaned, now);
            return cleaned;
        },
