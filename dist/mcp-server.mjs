...
function buildInstructions(store, config2) {
...
function createMcpServer(store, config2) {
  ...
  server.tool(
    ...
    async ({ name, category, organization, context, profession }) => {
      ...
      if (err) return respond(err);
      try {
        const result = createDossier(store, config2.crmRoot, { name, category, organization, context, profession });
  ...
  server.tool(
    ...
    async ({ remote }) => {
      ...
      if (remote) {
        try {
          const remoteTemplates = await listRemoteTemplates(config2.templateRepo, config2.crmRoot, config2.githubToken);
...
async function startMcpServerUnconfigured(config2) {
  const server = createMcpServer(null, config2);
  const transport = new StdioServerTransport();
  await server.connect(transport);
}
