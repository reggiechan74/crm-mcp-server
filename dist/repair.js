...
function applyMoves(dossierDir, audit, fixCodes, applied, failed) {
    for (const finding of audit.findings.misplaced) {
        if (!fixCodes.includes(finding.code) && !fixCodes.includes('all'))
            continue;
        try {
...
function applyStale(dossierDir, audit, fixCodes, applied, failed) {
    for (const finding of audit.findings.stale) {
        ...
        try {
            ...
            const fieldRe = new RegExp(`^(${finding.field}:\\s*)${escapeRegex(finding.current)}\\s*$`, 'm');
...
// ── Public API ──────────────────────────────────────────────────────
export function runRepair(dossierDir, templateDir, audit, fixCodes) {
    const applied = [];
    const failed = [];
    // Count lines before
