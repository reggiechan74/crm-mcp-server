...
import { listLocalTemplates, ensureManifest, isCustomized, readTemplateInfo, computeContentHash, writeManifest, countFiles, } from './templates.js';
...
function buildInstructions(store, config) {
    ...
    const profRows = store.db.prepare('SELECT profession, COUNT(*) as count FROM contacts WHERE profession IS NOT NULL GROUP BY profession').all();
...
export function createMcpServer(store, config) {
    ...
    // ── 4. crm_connections ────────────────────────────────────────────────
    server.tool('crm_connections', 'Get relationship graph for a contact — shows who they\'re connected to and how.', {
        contact: z.string().describe('Contact name or dossier code'),
    ...
    // ── 8. crm_log ───────────────────────────────────────────────────────
    server.tool('crm_log', 'Append a new interaction to a contact\'s log. Adds a row to the interaction table in log.md.', {
        contact: z.string().describe('Contact name or dossier code'),
    ...
    // ── 9. crm_vector_search ────────────────────────────────────────────
    server.tool('crm_vector_search', "Semantic search across all dossier content. Finds contacts and sections matching a natural language query. Requires embeddings (run 'crm-mcp embed' first).", {
    ...
    // ── 10. crm_create ─────────────────────────────────────────────────
    server.tool('crm_create', 'Create a new contact dossier from template.', {
    ...
    // ── 11. crm_bulk_update ─────────────────────────────────────────────
    server.tool('crm_bulk_update', 'Update a field across multiple contacts matching a filter.', {
        category: z.string().optional().describe('Filter by category'),
...
export async function startMcpServerUnconfigured(config) {
