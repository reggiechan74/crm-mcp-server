 ...
 */
export async function listRemoteTemplates(repo, crmRoot, token) {
    // Check cache
    ...
    const cache = { fetchedAt: new Date().toISOString(), templates };
 ...
 */
export async function downloadTemplate(repo, templateName, destDir, token, category) {
    ...
    try {
        ...
        try {
            execSync(`tar xzf "${tmpFile}" -C "${tmpExtract}" ${extractPaths.map(p => `"${p}"`).join(' ')}`, { encoding: 'utf-8' });
        }
        catch {
